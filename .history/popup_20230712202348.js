document.addEventListener('DOMContentLoaded', function () {
    ...
    toggleButton.addEventListener('click', async function() {
      ...
      chrome.storage.sync.get('blockerState', async function(data) {
        let newState = (data.blockerState === 'on') ? 'off' : 'on';
        
        // Update the state in storage
        chrome.storage.sync.set({'blockerState': newState}, async function() {
          ...
          if (newState === 'on') {
            await chrome.declarativeNetRequest.updateDynamicRules({addRules: blockingRules});
          } else {
